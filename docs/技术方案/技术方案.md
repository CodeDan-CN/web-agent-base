...
**技术栈：** Python 3.11+ + FastAPI + PostgreSQL + Tortoise ORM
...
PostgreSQL 可以稳定保存关系数据，降低第一版工程复杂度。

后续如果业务确实需要跨会话或长期上下文召回，可以通过独立记忆服务或 agentic RAG 服务扩展；这不进入第四阶段。
...
```mermaid
erDiagram
    SESSION_STATE ||--o{ AGENT_EVENT : "user_id + agent_id + session_id"
    AGENT_EVENT ||--o{ AGENT_EVENT_CONTEXT : "event_id"
    AGENT_EVENT ||--o{ AGENT_RUN : "event_id"
    ...
    AGENT_EVENT }o..o{ USER_LONG_TERM_MEMORY : "semantic recall"
...
`CONVERSATION_RECORD` 记录每条对话消息，不再承担检索召回。它的唯一业务键是 `user_id + agent_id + session_id + record_id`。

`AGENT_EVENT` 记录单轮或多轮用户交互形成的一件事。

`AGENT_EVENT_CONTEXT` 记录同一事件下每个 agent 自己的事件摘要。main 和 worker 共享 `event_id`，但不共享同一条摘要。
...
事件不是 Loop 状态，而是“用户这一件事”的上下文容器。一个事件可以覆盖单轮或多轮对话。

main Agent 和 worker Agent 共享同一个事件。

main 调用 worker 时，worker 不创建独立事件，而是使用 main 当前 `event_id`。worker 的 run、step 和 tool_call 都归属这个事件。

这样 worker Context Engineer 可以按 `event_id` 取到同一事件下的 main 历史、worker 历史和 Skill 调用结果，避免在同一事件内遗忘前面已经执行到哪里。
...
CREATE UNIQUE INDEX uq_agent_event_id
ON agent_event (event_id);

CREATE INDEX idx_agent_event_scope
ON agent_event (user_id, agent_id, session_id, status, created_at);
```

| 字段 | 说明 |
| --- | --- |
| `event_id` | 事件唯一 ID |
| `parent_event_id` | 当前事件基于某个前序事件继续追问或改写时记录 |
| `event_chain_id` | 同一串相关事件的链路 ID |
| `status` | `open / awaiting_user / completed / failed` |
| `title` | 事件短标题，可为空 |

#### agent_event_context

保存同一事件下每个 agent 自己的事件上下文摘要。

```sql
CREATE TABLE agent_event_context (
    id             BIGSERIAL PRIMARY KEY,
    event_id       VARCHAR(128) NOT NULL,
    user_id        VARCHAR(64) NOT NULL,
    agent_id       VARCHAR(64) NOT NULL,
    ...
    summarize_mode VARCHAR(32) NOT NULL DEFAULT 'direct',
    metadata       JSONB,
    created_at     TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at     TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

CREATE UNIQUE INDEX uq_agent_event_context_scope
ON agent_event_context (event_id, agent_id);

CREATE INDEX idx_agent_event_context_event
ON agent_event_context (event_id, agent_id, updated_at);
```

| 字段 | 说明 |
| --- | --- |
| `event_id` | 事件 ID，与 main / worker 共享 |
| `agent_id` | 当前摘要所属 agent，例如 `main`、`amap_worker` |
| `summary` | 当前 agent 在该事件内的上下文摘要 |
| `turn_count` | 用于判断是否需要总结的轮数 |
| `char_count` | 用于判断是否需要总结的字符数 |
| `summarize_mode` | `direct / llm`，表示直接拼接或模型总结 |

摘要生成策略：

1. 事件进行中先持续收集当前 `event_id + agent_id` 下的对话与执行内容。
2. 当主 Agent Harness 或主 Agent 最终状态认为事件结束时，Runtime 触发事件摘要更新。
3. 如果轮数和字数未超过限制，直接把该 agent 的事件内对话与关键执行结果汇总到 `summary`。
4. 如果超过限制，调用模型总结为约 100 字摘要。
5. main 和 worker 分别生成自己的 `agent_event_context.summary`，不互相覆盖。
...
CREATE INDEX idx_agent_run_event
ON agent_run (event_id, created_at);
```
...
`tool_call.event_id` 用于快速按事件聚合 main 调 worker、worker 调 Skill 的完整执行轨迹。
...
第一版通过进程内轻量 Runtime Hook 处理这些事件，例如写入 `conversation_record`、写入 `agent_run_step`、更新 `agent_event_context.summary` 和写日志。
...
Context Engineer 需要同时使用以下对话上下文：

- 最近若干条 `conversation_record`，用于保留当前会话的连续性。
- 当前 `event_id + agent_id` 对应的 `agent_event_context.summary`，用于找回当前 agent 在同一事件内的上下文。
- 当前事件的 main / worker run、step、tool_call 历史，用于让 worker 继续同一事件内已经执行过的任务。

Loop 开始前需要增加一层事件上下文预加载。

预加载发生在 Runtime 已经解析出 `user_id / session_id / agent_id / event_id` 之后，LoopDecider 调用之前。

它只负责把命中事件内与当前 agent 相关的一轮或多轮对话放进 Prompt，不负责判断下一步 Action。

```text
RuntimeRequest 进入
  ↓
EventManager 解析或创建当前事件
  ↓
EventContextPreloader 读取或匹配事件上下文
  ↓
...
1. 如果本轮请求已经带有 `event_id`，并且数据库能查到该事件，直接读取该事件对应的对话、summary 和必要执行摘要。
   summary 和执行摘要只做内部定位依据。
   Prompt 只注入该事件内与当前 agent 相关的一轮或多轮对话。
2. 如果本轮没有明确 `event_id`，但当前 `user_id + session_id` 下存在历史事件，先取最近若干个候选事件，再由模型判断本轮请求是否与其中某个事件相关或可能相关。
3. 如果模型判断相关，只注入一个最相关事件内与当前 agent 相关的一轮或多轮对话。
4. 如果当前会话没有历史事件，或者模型判断不相关，则不注入事件内对话。

模型相关性判断只接收候选事件的短标题、summary、状态和更新时间，不接收全量历史对话。
...
- 只注入当前用户、当前会话、当前 agent 相关的命中事件内一轮或多轮对话内容。
- 第三阶段不做跨用户、跨会话或全局长期记忆检索。
- 不注入命中事件的 `event_id`、标题、summary、worker / Skill 执行摘要、API Key、完整内部 Prompt、全量工具原始响应和数据库 ORM 原始对象。
- worker 不重新判断事件归属，只使用 main 传入的 `event_id`，读取 `event_id + worker_agent_id` 的上下文。

事件 summary 在记忆检索环节生成或更新。
...
event_id + agent_id
  ↓
读取当前 agent 的事件内对话和执行历史
  ↓
未超过轮数和字数限制：直接汇总为 summary
  ↓
超过限制：调用模型总结为约 100 字 summary
  ↓
写入 agent_event_context.summary
  ↓
注入当前 agent 的 Context
```

第三阶段允许在当前会话内做轻量事件相关性判断，用来识别“用户是否在追问上一件事”。

更大范围的跨会话、跨事件检索不进入第四阶段，后续如有需要再作为独立记忆服务或个人助手型 Agent 能力演进。

worker Context Engineer 使用 main 当前 `event_id` 获取上下文。

worker 不拥有独立事件，也不直接决定事件关闭。事件关闭由 Runtime 根据主 Agent Harness 或主 Agent 最终完成状态推动。

同一事件下，main 和 worker 的 summary 按 `agent_id` 分开保存。worker 读取自己的 summary，也可以由 Runtime 按需补充 main 的事件摘要。

记忆宫殿不进入业务型 Agent 框架的第四阶段。

相关服务协议、管理接口和召回策略记录到 [技术债务.md](./技术债务.md)，作为个人助手型 Agent 框架或更完整记忆服务的后续演进。
...
- 如果执行层已经成功，但结果与用户真实意图不一致，或仍有地址、对象、范围、时间、出行方式等可补救歧义，必须输出 `missing_params`，由 Loop 决定继续补齐还是追问用户。
...
| `AssistantMessageGenerated` | 最终回答生成 | 写入 `conversation_record(role = assistant)`，必要时更新当前 `agent_event_context.summary` |
...
| 表 | 内容 |
| --- | --- |
| `conversation_record` | 用户和 Agent 的原始对话记录 |
| `agent_run` | 一次请求总记录 |
...
- Worker Task Package 传递当前请求、任务、自然语言交接信息、event_id、轻量会话摘要、最近几轮对话和同一事件执行历史拼接位置。
- agent_event / agent_event_run / agent_event_context 可写入。
- 事件结束时通过简易 Hook 生成或更新 main / worker 各自的事件 summary。
- main / worker 进入 Loop 前执行 EventContextPreloader。
- 请求明确带有已存在 `event_id` 时，直接把该事件内与当前 agent 相关的一轮或多轮对话拼接进 Prompt。
- 请求未明确绑定事件但当前会话存在历史事件时，由模型判断是否相关，相关则拼接命中事件内与当前 agent 相关的一轮或多轮对话。
- 当前会话无历史事件时，不执行事件相关性判断。
- 第三阶段只预留 `retrieved_context`、`long_term_memory`、`memory_palace_refs` 字段，不实现完整检索和压缩。

### 第四阶段：会话上下文压缩闭环

目标：在当前 session 内，通过 token 预算驱动的摘要压缩、最近对话窗口和事件内对话去重，保证业务 Agent 长会话不中断、不重复、不越界。

交付：

- 当前会话对话记录可持久化。
- 每个 `conversation_turn` 表示一轮用户可见输入和最终回答，拥有稳定 `turn_id` 或等价 run 标识。
- 基于 `user_id + agent_id + session_id` 维护会话上下文。
- 引入 `SESSION_CONTEXT_MAX_TOKENS` 等上下文预算配置，第一版默认历史上下文窗口为 6000 tokens。
- 预算未达阈值时不做模型摘要。
- 未压缩 raw `conversation_turns` 达到约 4800 tokens 时，将较早对话压缩进 `session_state.conversation_summary`。
- 压缩后未压缩 raw turns 尽量回到约 2100 tokens。
- 保留最近 4 到 8 轮原始对话作为 `recent_turns`，最近 4 轮必须保护。
- 第三阶段已有的 `EventContextPreloader` 继续提供 `event_dialogues`。
- Context Engineer 对 `recent_turns` 和 `event_dialogues` 按 `conversation_turn` 做纯代码精确去重。
- Prompt 拼装形成 `conversation_summary + recent_turns + event_dialogues_deduped + current_user_message` 四层结构。
- main 和 worker 的会话摘要按 `agent_id` 隔离。
- 不接入记忆宫殿。
- 不接入 `memory_manager`。
- 不做跨用户或跨 session 召回。
