...
更大范围的跨会话、跨事件检索仍由后续更完整的记忆服务或 `memory_manager` 基于事件 summary 处理。
...
目标：长期记忆、事件 summary、记忆宫殿 Provider 和 `memory_manager` 子 Agent 进入记忆闭环。
...
- 每轮用户输入和 Agent 回答可保存。
- 第三阶段已有的事件 summary 和事件上下文预加载继续可用。
- 未超过轮数和字数限制时直接汇总，超过限制时由模型总结为约 100 字的策略可继续优化。
- LLM 对话压缩摘要可用。
