...
当 worker 的 `output_contract.final_answer_owner = main_agent` 时，worker 进入可回答状态后，不再额外生成一轮面向用户的回答文本；`answer_user` 只作为“领域结果已可返回”的完成信号，由 Runtime 直接把结构化结果回传给 main。
...
- 如果 worker 的最终回答权归 main，则 worker 的 `answer_user` 是完成信号，不是再做一次用户向文案生成。
...
- 如果 `final_answer_owner = main_agent`，输出重点是结构化结果和摘要，不再追加一轮面向用户的自然语言回答。
